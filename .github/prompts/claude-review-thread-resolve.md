...
gives you, with an entry in `results` for EVERY thread index you were given:
...
Resolving the last thread carrying a 🔴/🟡 finding is what turns the merge gate
green, so a false positive here can let a PR merge past a real concern: when in
doubt, `false`.
