...
| `scan-loaded-instructions` | InstructionsLoaded | Scans each instruction file Claude Code loads later — a subdirectory's `CLAUDE.md`, a path-scoped rule, an `@import`, a post-compaction reload — from the bytes the event hands it, so nothing below the launch set needs a tree walk; auto-cleans in-project findings and tells the model the file it just read is untrusted                                                                                                                                               |
