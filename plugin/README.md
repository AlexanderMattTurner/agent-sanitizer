...
| `scan-loaded-instructions` | InstructionsLoaded | Scans each instruction file Claude Code names as it loads it — a subdirectory's `CLAUDE.md`, a path-scoped rule, an `@import`, a post-compaction reload, the user-global `~/.claude/CLAUDE.md` and global rules — reading the one path the event names, so nothing below the launch set needs a tree walk; auto-cleans in-project findings, tells the model the file it just read is untrusted, and reports a `.claude/` directory loading as context that the launch scan prunes                         |
...
| `AGENT_SANITIZER_TERMINAL_DISABLED=1`  | Keep every raw escape sequence, not just colour (escape fixtures)  |
...
Four knobs for how the launcher finds its runtime, reuses its compiled code, and reports both:
...
| `AGENT_SANITIZER_COMPILE_CACHE=0`           | Never keep V8's compiled code for the hook bundle. By default the launcher caches it under `$CLAUDE_PLUGIN_DATA/node-compile-cache`, so the ~2.4 MB bundle is compiled once per install instead of once per tool call; the directory is created owner-only and skipped when it is not, because V8 does not treat a code cache as untrusted input. An operator's own `NODE_COMPILE_CACHE` is used unchanged. |
