...
function stageProvisionable(
  ...
  // The real digest of the staged bundle, not a placeholder: the provisioner
  // refuses a manifest whose `# bundle=` does not describe the bundle this
  // install ships, so a fixture pinning zeroes would model a checkout the
  // provisioner is supposed to decline — and every case here would assert
  // against that refusal instead of the behaviour it names.
  writeFileSync(manifest, stagedManifest(plugin, { digest, asset }));
  return { dir, plugin, fixture, manifest, data: join(dir, "data") };
}

/** The bundle the staged plugin ships, as the provisioner hashes it. */
function bundleDigest(plugin) {
  return createHash("sha256")
    .update(
      readFileSync(join(plugin, "dist", "hooks", "plugin-hooks.bundle.mjs")),
    )
    .digest("hex");
}

/** The manifest text for a staged plugin, in the generator's own shape. */
function stagedManifest(
  plugin,
  { digest, asset, bundle = bundleDigest(plugin) },
) {
  return (
...
test("a planted binary with a matching stamp is removed, not trusted", (t) => {
  // The attack the fast path has to survive: the install dir is writable by
  // whatever pre-created it, and the stamp is a copy of a PUBLIC committed
  // file — so an attacker who plants a binary can plant the stamp beside it.
  // A fast path that compares the manifest with that copy exits 0 and every
  // hook in the session then execs the planted file.
  const staged = stageProvisionable(t);
  const { path, curlLog } = provisionPath(t, staged.fixture);
  const dir = join(staged.data, "hook-binary");
  const binary = join(dir, "agent-sanitizer-hooks");
  mkdirSync(dir, { recursive: true });
  writeFileSync(binary, "#!/bin/sh\necho PWNED\n", { mode: 0o755 });
  cpSync(staged.manifest, join(dir, ".manifest-installed"));

  const res = provision(staged, path);
  assert.equal(res.status, 0, res.stderr);
  assert.match(res.stderr, /REPLACED after it was installed/);
  // Healed from the release, not merely warned about: what the launcher execs
  // must be the manifest's own bytes.
  assert.equal(readFileSync(binary, "utf8"), "FAKE HOOK BINARY\n");
  assert.equal(readFileSync(curlLog, "utf8").trim().split("\n").length, 1);
});

test("a planted binary is removed even when the release cannot be reached", (t) => {
  // The half that decides whether the plant ever runs: healing needs the
  // network, refusing does not. A provisioner that only overwrites the plant
  // on a successful download leaves it executable on exactly the offline host
  // where nothing else will notice.
  const staged = stageProvisionable(t);
  const { path, curlLog } = provisionPath(t, staged.fixture);
  writeFileSync(
    join(path, "curl"),
  ...
  assert.match(res.stderr, /REPLACED after it was installed/);
  assert.ok(!existsSync(binary), "the planted binary survived the refusal");
});

test("the fast path repairs an install dir anything could write", (t) => {
  // 0777 is what a pre-created dir keeps forever if only the slow path chmods
  // it: the first session installs through the slow path, every later one
  // takes the fast path, and the mode nobody repaired is what lets the plant
  // above happen in the first place.
  const staged = stageProvisionable(t);
...
test("a manifest that does not describe this install's bundle provisions nothing", (t) => {
  // The manifest's `# bundle=` pins the bundle the release binaries were
  // compiled FROM. When the committed bundle has moved since the last manifest
  // regen, the binary that verifies against this manifest is NOT this
  // checkout's sanitizer — and the launcher prefers the binary over the
  // bundle, so provisioning would guard the session with code the install does
  // not contain.
  const staged = stageProvisionable(t);
  const bundle = join(
    staged.plugin,
  ...
  const res = provision(staged, path);
  assert.equal(res.status, 1);
  assert.match(res.stderr, /compiled from a DIFFERENT bundle/);
  assert.ok(!existsSync(curlLog), "it downloaded a binary it could not accept");

  // Non-vacuity: the same tree with the manifest regenerated does provision,
  // so the refusal is about the digest and not about the staging.
  writeFileSync(
    staged.manifest,
    stagedManifest(staged.plugin, {
      digest: createHash("sha256").update("FAKE HOOK BINARY\n").digest("hex"),
      asset: ASSET,
    }),
  );
  assert.equal(provision(staged, path).status, 0);
...
test("a manifest with no bundle digest provisions nothing", (t) => {
  const staged = stageProvisionable(t);
  writeFileSync(
    staged.manifest,
  ...
  const res = provision(staged, path);
  assert.equal(res.status, 1);
  assert.match(res.stderr, /no usable bundle digest/);
