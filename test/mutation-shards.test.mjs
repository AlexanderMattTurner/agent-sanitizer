...
describe("the dry-run oracle's scope", () => {
  ...
  it("prints the shipped surface bare, and the mutated set under --mutated", () => {
    ...
    // The two must actually differ, or every assertion here and the wiring pin
    // below would hold with the oracle still instrumenting a subset.
    assert.ok(
  ...
  it("asks for the scope the shards mutate", () => {
    ...
    // a subset is invisible — the job passes and a shard dies on a dry-run
