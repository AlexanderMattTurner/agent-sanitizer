...
fails the hook at all: only the colliding fields are withheld, under a
disambiguated name that keeps the object's field count, so the sanitized output
is still shape-valid and every sibling field survives. A hostile connector can
