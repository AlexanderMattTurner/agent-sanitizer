...
- **"Unverified" is a claim about a command, never a bare adjective.** Never write "unverified", "couldn't measure", "no access", or "can't run here" without (1) naming the exact command, workflow or job that would produce the measurement, (2) actually attempting it, and (3) pasting the literal failure output. A local check does not stand in for the real one: this container lacking `/dev/kvm` says nothing about whether CI has it, and a workflow's own logs are the cheapest way to find out — read them before claiming the number does not exist. Worked example: _"`bin/checks/kata/teardown-timing.py` measures this; `kata-metrics-probe.yaml` already runs it on the PR path — run 35043814270 printed `teardown_total: 0.663`."_ Not: _"target unverified, no KVM available."_ If the attempt genuinely fails, the failure text is the deliverable — `HTTP 403: Dispatching ... not permitted for this session type` is actionable; "unverified" is not. <!-- allow-workflow-ref: the worked example names a sibling repo's (agent-glovebox) Kata probe workflow, which this tree can never resolve. -->
- **Standing down on a base-branch failure is about this PR's scope, never about the fix.** "Not this PR's failure" means **open a separate PR**, not do nothing. The PR-hygiene rule that keeps an unrelated fix out of the current diff does not cancel the follow-through rule above: diagnose the root cause, then land it somewhere — a sibling branch and its own PR — and link that PR from the comment where you stood down. A diagnosis posted with no fix PR behind it leaves the repo exactly as broken as before you looked.
