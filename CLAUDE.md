...
- Add the `ci:mutation` label to PRs that change a mutation-gated source or the tests that kill its mutants — the shards otherwise run only on the daily schedule against `main`, so a regression surfaces after the merge rather than before it.
