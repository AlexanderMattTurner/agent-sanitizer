...
- **"Unverified" is a claim about a command, never a bare adjective.** Never write "unverified", "couldn't measure", "no access", or "can't run here" without (1) naming the exact command, workflow or job that would produce the measurement, (2) actually attempting it, and (3) pasting the literal failure output, **redacted** — a token, an `Authorization` header, a signed URL or any other credential in the invocation or its stderr is replaced with `<redacted>` before the text goes into chat, a commit, or a PR. The status line and the error identity are what make the evidence useful; the secret never is. A local check does not stand in for the real one: this container lacking `/dev/kvm` says nothing about whether CI has it, and a workflow's own logs are the cheapest way to find out — read them before claiming the number does not exist. Worked example: _"`bin/checks/kata/teardown-timing.py` measures this; `kata-metrics-probe.yaml` already runs it on the PR path — run 35043814270 printed `teardown_total: 0.663`."_ Not: _"target unverified, no KVM available."_ If the attempt genuinely fails, the failure text is the deliverable — `HTTP 403: Dispatching ... not permitted for this session type` is actionable; "unverified" is not. <!-- allow-workflow-ref: the worked example names a sibling repo's (agent-glovebox) Kata probe workflow, which this tree can never resolve. -->
- **Standing down on a base-branch failure is about where the fix lands, never about whether it lands.** "Not this PR's failure" is never a licence to do nothing. Which branch carries it is settled, in this order: a check **red on your own PR** is governed by `.claude/skills/ci-triage/SKILL.md` — root-cause it and fix it **on this branch**, in its own `fix(ci):`/`fix(test):` commit, however unrelated it is. A separate PR is for the breakage `ci-triage` cannot reach: one in **another repo**, or one no commit on this branch can address — and there, check for an open PR already covering it first (per the open-PR bullet below) and push to that branch rather than starting a sibling. Diagnose the root cause, land it in whichever of those places applies, and link it from the comment where you stood down. A diagnosis with no fix behind it leaves the repo exactly as broken as before you looked.
...
**`recheck-review-gate` is THIS repo's spelling, and `gh pr edit <N>` means THIS repo's PR #N. Acting on another repo, read its label name off the failing status and pass `-R` on both halves.** The name is hard-coded per repo and each repo hides it somewhere different — here `review-findings-gate.yaml` writes `recheck-review-gate` inline, while `agent-glovebox`'s `consume-review-gate-recheck.sh` resolves `review-gate-recheck` through its own `shared-names.json`. Neither is reachable from the other, `git grep` cannot find a string you do not already know, and `.pr_labels` carries the gate name in some repos and not others (not this one). **The failing status is the non-circular source: its description reads `resolve each thread, then REMOVE and re-add the <label> label`.** Take the name from there. Then write both halves as `gh pr edit <N> -R <owner>/<repo> --remove-label <label>` and `--add-label <label>` — without `-R`, `gh` resolves `<N>` against the repo you are standing in, relabelling an unrelated PR here while the target's gate stays stale. A wrong name is not a clean no-op either: the gate subscribes to `labeled` generically, with no name condition, so the wrong label fires ONE evaluation that re-posts the verdict already standing, and then the name-specific cleanup step never removes it — the label sticks, and since `labeled` fires only on a transition, re-adding it is dead forever. The hatch reads as working and is not.
...
- Mutation shards run only on the daily schedule against `main` (or a manual `workflow_dispatch`), never on a PR — a regression surfaces after the merge rather than before it. There is no PR opt-in label for this; dispatch the workflow by hand for a pre-merge verdict.
