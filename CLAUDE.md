...
- **Write every quantity in the unit that makes it read naturally, never in scientific notation or another unit's fractions.** `850ms`, not `0.85s` or `8.5 × 10⁻¹ s`; `2.4s`, not `2400ms`; `1m49s`, not `109s`; `4.7 MB`, not `4700000 B`. This covers anything a human reads—log lines, step summaries, error messages, docs, comments—so a formatter that picks the unit by magnitude belongs next to the measurement, not repeated at each call site. Scientific notation is right only where it genuinely is the natural form (a tolerance like `1e-9`, an axis spanning many decades).
