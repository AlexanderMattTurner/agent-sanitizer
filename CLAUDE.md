...
**`recheck-review-gate` is THIS repo's spelling. Acting on another repo, resolve the label name from that repo's SSOT first.** A gate label is repo-local config, not a protocol: each repo names its own under `.pr_labels` in `.github/scripts/lib/shared-names.json`, and the consuming workflow's `labeled` filter matches that one string. A name the target repo does not use fails silently in the worst direction — GitHub auto-creates the missing label (grey, no description), the PR reads as relabelled, and no gate ever re-runs; a no-op label is indistinguishable from a gate that ran and found nothing. Before the two `gh pr edit` halves, read the target's `shared-names.json` (or `git grep` the label string in its workflows) and use what it returns. Worked example: `agent-glovebox` spells that key `review_gate_recheck: "review-gate-recheck"`, so both halves there name `review-gate-recheck`; carrying this repo's `recheck-review-gate` across instead put an inert label on 41 PRs and re-ran nothing.
