...
**A merge-delta finding thread on your PR is YOURS to settle — it does not wait for the user.** Nothing resolves a review thread for you. Push a corrected resolution and a clean re-review clears it; when the resolution is right, reply with the parent-tracing evidence per flagged hunk (`git show --remerge-diff <merge>`, `git diff <parent>..HEAD -- <file>` — name which parent every surviving line came from), then resolve. Never resolve without that reply. **Regenerating a lockfile and diffing is not evidence**: `uv lock` preserves the entries already committed, so it reproduces forged bytes faithfully; the hash is checked at `uv sync`.
